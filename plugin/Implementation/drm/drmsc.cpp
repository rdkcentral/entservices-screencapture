...
bool DRMScreenCapture_GetScreenInfo(DRMScreenCapture* handle) {
	...
	do {
		...
		for (int attempt = 0; attempt < maxAttempts; ++attempt) {
			...
			uint32_t plane_id = (context->kms->topmost_plane_id != KMS_INVALID_PLANE_ID)
				? context->kms->topmost_plane_id : context->kms->primary_plane_id;

			if (plane_id != KMS_INVALID_PLANE_ID) {
		...
		// plane and fb are only ever set together (see loop above), so a single check suffices
		if(!plane || !fb) {
			cout << "[SCREENCAP] fail to select an active plane/framebuffer after retries" << endl;
			ret = false;
			break;
		}
